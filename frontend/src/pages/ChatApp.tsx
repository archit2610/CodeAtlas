...
export default function ChatApp() {
  ...
  useEffect(() => {
    ...
    if (paramConvoId) {
      ...
      api.get<Report>(`/api/v1/reports/report/${paramToken}`).then((res) => {
  ...
  const startAnonymousStream = async (q: string, targetConvoId?: string) => {
    try {
      const response = await fetch(`${API_URL}/api/v1/reports/research`, {
        method: 'POST',
  ...
  const startSSE = (reportToken: string, targetConvoId?: string) => {
    closeSSE();
    const url = `${API_URL}/api/v1/reports/report/${reportToken}/run`;
  ...
  const handleSubmitPrompt = async (e?: FormEvent, customPrompt?: string) => {
    ...
    try {
      const res = await api.post<{ report?: { id: string }; conversationId?: string }>('/api/v1/reports/create', {
        question: q,
