...
export const agentRuns = pgTable('agent_runs', {
    id: uuid('id').primaryKey().defaultRandom(), repositoryId: uuid('repository_id').notNull().references(() => repositories.id, { onDelete: 'cascade' }), visitorId: text('visitor_id').notNull(), conversationId: uuid('conversation_id').references(() => conversations.id, { onDelete: 'set null' }), request: text('request').notNull(), intent: text('intent').notNull(), status: text('status').notNull().default('pending'), evidenceJson: jsonb('evidence_json').$type().notNull().default([]), planJson: jsonb('plan_json').$type(), riskJson: jsonb('risk_json').$type(), answerMd: text('answer_md'), patchText: text('patch_text'), reviewMd: text('review_md'), tokensUsed: integer('tokens_used'), costUsd: real('cost_usd'), errorMessage: text('error_message'), createdAt: timestamp('created_at').defaultNow().notNull(), updatedAt: timestamp('updated_at').defaultNow().notNull(),
