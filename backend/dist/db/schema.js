...
export const repositories = pgTable('repositories', {
    ...
    languages: jsonb('languages').$type().notNull().default({}),
    frameworks: jsonb('frameworks').$type().notNull().default([]),
    snapshotJson: jsonb('snapshot_json').$type().notNull().default({}),
...
export const repositoryFiles = pgTable('repository_files', {
    ...
    symbolsJson: jsonb('symbols_json').$type().notNull().default([]),
    importsJson: jsonb('imports_json').$type().notNull().default([]),
    createdAt: timestamp('created_at').defaultNow().notNull(),
}, (table) => [
...
export const agentRuns = pgTable('agent_runs', {
    ...
    status: text('status').notNull().default('pending'),
    evidenceJson: jsonb('evidence_json').$type().notNull().default([]),
    planJson: jsonb('plan_json').$type(),
    riskJson: jsonb('risk_json').$type(),
