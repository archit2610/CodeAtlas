...
export const searchRepository = async (repositoryId, query) => {
    const q = query.trim();
    if (!q)
        return [];
    const matches = await db.select({
        ...
        .where(and(eq(repositoryFiles.repositoryId, repositoryId), or(ilike(repositoryFiles.path, `%${q}%`), ilike(repositoryFiles.content, `%${q}%`))))
        .limit(10);
...
export const readFileLines = async (repositoryId, filePath, startLine = 1, endLine = 100) => {
...
//# sourceMappingURL=repository-tools.service.js.map