...
export const calculateBlastRadius = async (repositoryId, filePath) => {
    ...
    if (directPaths.length > 0) {
        const secondaryEdges = await db.select({ fromPath: repositoryEdges.fromPath })
            .from(repositoryEdges)
            .where(and(eq(repositoryEdges.repositoryId, repositoryId), inArray(repositoryEdges.toPath, directPaths)));
    ...
    if (allImpactedPaths.length > 0) {
        const routeFiles = await db.select({ path: repositoryFiles.path })
            .from(repositoryFiles)
            .where(and(eq(repositoryFiles.repositoryId, repositoryId), eq(repositoryFiles.classification, 'route'), inArray(repositoryFiles.path, allImpactedPaths)));
