...
export const resolveImportPath = (fromPath, target, knownPaths) => {
...
export const scanFile = (filePath, content) => {
    const normalizedPath = filePath.replaceAll('\\', '/');
    const language = detectLanguage(normalizedPath);
    const symbols = [];
    const imports = [];
    const addSymbol = (r, kind) => {
...
export const buildSnapshot = (files) => {
    const languages = {};
    ...
    const by = (p) => files.filter(p).map(f => f.path);
