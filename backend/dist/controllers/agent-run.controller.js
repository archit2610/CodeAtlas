...
export const getAgentRunEvents = asyncHandler(async (req, res) => {
    ...
    if (run.status === 'completed') {
        ...
        if (run.patchText)
            emit({ type: 'patch', patchText: run.patchText, reviewMd: run.reviewMd });
...
export const getPatch = asyncHandler(async (req, res) => {
    const runId = req.params.id;
    const visitorId = req.guestTempId;
