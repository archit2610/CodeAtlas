...
import { createConversation, getConversationById } from "../services/conversation.service.js";
export const runReport = asyncHandler(async (req, res) => {
    const { token } = req.params;
    if (!token)
        throw new ApiError(400, "Report token is required");
    ...
    if (!conversationId) {
        const convo = await createConversation(req.guestTempId, report.question);
        if (!convo)
            throw new ApiError(400, "Failed to create conversation");
        conversationId = convo.id;
        await updateReport(reportId, { conversationId });
    }
    else if (!await getConversationById(conversationId, req.guestTempId)) {
        throw new ApiError(404, "Report not found");
    }
