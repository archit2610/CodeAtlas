...
import { createReport, getReportById, updateReport, deletereport } from "../services/report.service.js";
...
export const createResearch = asyncHandler(async (req, res) => {
    ...
    if (!conversationId) {
        const convo = await createConversation(req.guestTempId, question.trim());
        if (!convo)
